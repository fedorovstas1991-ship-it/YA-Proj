...
import { DEFAULT_CRON_WIZARD } from "../app-defaults.ts";
import type { CronFormState, CronWizardState } from "../ui-types.ts";
import { toNumber } from "../format.ts";
import { buildCronJobFromWizard } from "./cron-wizard.ts";
...
export async function addCronJobFromWizard(state: CronState & { cronWizard: CronWizardState }) {
  if (!state.client || !state.connected || state.cronBusy) {
    return;
  }
  state.cronBusy = true;
  state.cronError = null;
  state.cronWizard.error = null;
  try {
    const job = buildCronJobFromWizard(state.cronWizard);
    await state.client.request("cron.add", job);
    state.cronWizard = {
      ...DEFAULT_CRON_WIZARD,
      mode: state.cronWizard.mode,
      scenario: state.cronWizard.scenario,
      schedulePreset: state.cronWizard.schedulePreset,
      inAmount: state.cronWizard.inAmount,
      inUnit: state.cronWizard.inUnit,
      dailyTime: state.cronWizard.dailyTime,
      weekDays: [...state.cronWizard.weekDays],
      weekTime: state.cronWizard.weekTime,
      step: 1,
      error: null,
      text: "",
    };
    ...
    state.cronError = `Не удалось добавить задачу: ${details}`;
    state.cronWizard = {
      ...state.cronWizard,
      error: `Не удалось добавить задачу. Проверьте поля и попробуйте снова.\n${details}`,
    };
  } finally {
    state.cronBusy = false;
  }
