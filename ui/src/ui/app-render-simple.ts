...
function renderConnectionPanel(state: AppViewState) {
  ...
  return html`
    <section class="card simple-setup-card">
      <div class="card-title">1. Подключение к локальному OpenClaw</div>
      <div class="card-sub">
        Репозиторий уже есть, шаг скачивания пропущен. Здесь только подключение и запуск онбординга.
      </div>
      ...
      <aside class="simple-sidebar">
        <div class="simple-brand">
          <div class="simple-brand__mark">
            <img src=${faviconHref} alt="OpenClaw" />
          </div>
          <div>
            <div class="simple-brand__title">OpenClaw</div>
            ...
            state.simpleDevToolsOpen
              ? html`
                <div class="simple-dev-links">
                  <a class="btn simple-nav-link" href=${buildTabHref("channels", state.basePath, state.sessionKey)}>
                    Телеграм и каналы
                  </a>
