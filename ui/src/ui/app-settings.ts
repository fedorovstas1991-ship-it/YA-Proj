...
export function applySettingsFromUrl(host: SettingsHost) {
  ...
  const clearLocalStorageRaw =
    params.get("clearLocalStorage") ?? hashParams.get("clearLocalStorage");
  let shouldCleanUrl = false;

  if (clearLocalStorageRaw != null) {
    const normalized = clearLocalStorageRaw.trim().toLowerCase();
    const shouldClear =
    ...
    if (shouldClear) {
      ...
      // After full localStorage reset we should open the canonical main chat.
      const mainSessionKey = "agent:main:main";
      host.sessionKey = mainSessionKey;
      host.applySessionKey = mainSessionKey;
    }
    params.delete("clearLocalStorage");
    hashParams.delete("clearLocalStorage");
    shouldCleanUrl = true;
  }
...
export function inferBasePath() {
  ...
  const configured = window.__YAGENT_CONTROL_UI_BASE_PATH__;
