...
function agentBadgeText(agentId: string, defaultId: string | null) {
  return defaultId && agentId === defaultId ? "по умолчанию" : null;
}
...
function buildModelOptions(configForm: Record<string, unknown> | null, current?: string | null) {
  ...
  if (current && !hasCurrent) {
    options.unshift({ value: current, label: `Текущая (${current})` });
  }
  if (options.length === 0) {
    return html`
      <option value="" disabled>Нет настроенных моделей</option>
    `;
  }
    ...
    <div class="agents-layout">
      <section class="card agents-sidebar">
        <div class="row" style="justify-content: space-between;">
          <div>
            ...
            <div class="card-sub">Настроено: ${agents.length}.</div>
          </div>
      ...
      : nothing
        ...
        <div class="agent-list" style="margin-top: 12px;">
          ${agents.length === 0
      ? html`
                  <div class="muted">Агенты не найдены.</div>
                `
      : agents.map((agent) => {
      ...
      ? html`
                <div class="card">
                  <div class="card-title">Выберите агента</div>
                  <div class="card-sub">Откройте агента, чтобы посмотреть workspace и инструменты.</div>
                </div>
              `
      : html`
              ${renderAgentHeader(
...
function renderAgentHeader(
  ...
  const subtitle = agent.identity?.theme?.trim() || "Рабочее пространство и маршрутизация агента.";
...
function renderAgentOverview(params: {
  ...
  return html`
    <section class="card">
      ...
      <div class="card-sub">Пути workspace и метаданные идентичности.</div>
      <div class="agents-overview-grid" style="margin-top: 16px;">
        <div class="agent-kv">
          <div class="label">Рабочее пространство</div>
          <div class="mono">${workspace}</div>
        </div>
        <div class="agent-kv">
          <div class="label">Модель ответа</div>
          <div class="mono">${model}</div>
        </div>
        <div class="agent-kv">
          <div class="label">Имя идентичности</div>
          <div>${identityName}</div>
          ${identityStatus ? html`<div class="agent-kv-sub muted">${identityStatus}</div>` : nothing}
        </div>
        <div class="agent-kv">
          <div class="label">По умолчанию</div>
          <div>${isDefault ? "да" : "нет"}</div>
        </div>
        <div class="agent-kv">
          <div class="label">Эмодзи идентичности</div>
          <div>${identityEmoji}</div>
        </div>
        <div class="agent-kv">
          <div class="label">Фильтр скиллов</div>
          <div>${skillFilter ? `Выбрано: ${skillCount}` : "все скиллы"}</div>
        </div>
      </div>

      <div class="agent-model-select" style="margin-top: 20px;">
        <div class="label">Настройка моделей</div>
      ...
      : html`
                      <option value="">
                        ${defaultPrimary ? `Наследовать (${defaultPrimary})` : "Наследовать по умолчанию"
        }
                      </option>
                    `
    }
          ...
          <label class="field" style="min-width: 260px; flex: 1;">
            <span>Запасные модели (если основная недоступна)</span>
            <input
              .value=${fallbackText}
              ?disabled=${!configForm || configLoading || configSaving}
...
function renderAgentContextCard(context: AgentContext, subtitle: string) {
  return html`
    <section class="card">
      <div class="card-title">Контекст агента</div>
      <div class="card-sub">${subtitle}</div>
      <div class="agents-overview-grid" style="margin-top: 16px;">
        <div class="agent-kv">
          <div class="label">Рабочее пространство</div>
        ...
        <div class="agent-kv">
          <div class="label">По умолчанию</div>
          <div>${context.isDefault ? "да" : "нет"}</div>
        </div>
      </div>
    </section>
  `;
...
function renderAgentChannels(params: {
  ...
  return html`
    <section class="grid grid-cols-2">
      ${renderAgentContextCard(context, "Workspace, идентичность и конфигурация модели.")}
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
            <div class="card-title">Каналы</div>
            <div class="card-sub">Снимок статуса каналов gateway.</div>
          </div>
          <button class="btn btn--sm" ?disabled=${params.loading} @click=${params.onRefresh}>
            ${params.loading ? "Обновляем…" : "Обновить"}
          </button>
        </div>
        <div class="muted" style="margin-top: 8px;">
          Последнее обновление: ${lastSuccessLabel}
        </div>
        ${params.error
      ...
      : nothing
        ...
        ${!params.snapshot
      ? html`
                <div class="callout info" style="margin-top: 12px">Загрузите каналы, чтобы увидеть актуальный статус.</div>
              `
      : nothing
    }
        ${entries.length === 0
      ? html`
                <div class="muted" style="margin-top: 16px">Каналы не найдены.</div>
              `
      : html`
        ...
        const status = summary.total
          ? `${summary.connected}/${summary.total} подключено`
          : "нет аккаунтов";
        const config = summary.configured
          ? `${summary.configured} настроено`
          : "не настроено";
        const enabled = summary.total ? `${summary.enabled} включено` : "выключено";
...
function renderAgentCron(params: {
  ...
  return html`
    <section class="grid grid-cols-2">
      ${renderAgentContextCard(context, "Workspace и цели расписания.")}
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
            <div class="card-title">Планировщик</div>
            <div class="card-sub">Статус cron gateway.</div>
          </div>
          <button class="btn btn--sm" ?disabled=${params.loading} @click=${params.onRefresh}>
            ${params.loading ? "Обновляем…" : "Обновить"}
          </button>
        ...
        <div class="stat-grid" style="margin-top: 16px;">
          <div class="stat">
            ...
            <div class="stat-value">
              ${params.status ? (params.status.enabled ? "Да" : "Нет") : "н/д"}
            </div>
          </div>
          <div class="stat">
            <div class="stat-label">Задач</div>
            <div class="stat-value">${params.status?.jobs ?? "н/д"}</div>
          </div>
          <div class="stat">
    ...
    <section class="card">
      <div class="card-title">Cron-задачи агента</div>
      <div class="card-sub">Плановые задачи для этого агента.</div>
      ${jobs.length === 0
      ? html`
              <div class="muted" style="margin-top: 16px">Задачи не назначены.</div>
            `
      : html`
              <div class="list" style="margin-top: 16px;">
                ${jobs.map(
