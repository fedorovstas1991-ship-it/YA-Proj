...
function createProps(overrides: Partial<CronProps> = {}): CronProps {
  return {
    ...
    wizard: { ...DEFAULT_CRON_WIZARD },
    channels: [],
    channelLabels: {},
    runsJobId: null,
    runs: [],
    onFormChange: () => undefined,
    onWizardChange: () => undefined,
    onWizardModeChange: () => undefined,
    onRefresh: () => undefined,
    onAdd: () => undefined,
    onAddWizard: () => undefined,
    onDraftInChatTemplate: () => undefined,
    onToggle: () => undefined,
    onRun: () => undefined,
    onRemove: () => undefined,
...
describe("cron view", () => {
  ...
  it("prompts to select a job before showing run history", () => {
    ...
    expect(container.textContent).toContain("Выберите задачу, чтобы посмотреть историю запусков.");
  });
  ...
  it("marks the selected job and keeps History button to a single call", () => {
    ...
    const historyButton = Array.from(container.querySelectorAll("button")).find(
      (btn) => btn.textContent?.trim() === "История",
    );
  ...
  it("shows selected job name and sorts run history newest first", () => {
    ...
    expect(container.textContent).toContain("Последние запуски для Daily ping.");

    const cards = Array.from(container.querySelectorAll(".card"));
    const runHistoryCard = cards.find(
      (card) => card.querySelector(".card-title")?.textContent?.trim() === "История запусков",
    );
