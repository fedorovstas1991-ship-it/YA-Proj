...
export function renderChannelConfigForm(props: ChannelConfigFormProps) {
  ...
  if (!normalized) {
    return html`
      <div class="callout warn">Схема конфига недоступна. Используйте режим Raw.</div>
    `;
  }
  const node = resolveSchemaNode(normalized, ["channels", props.channelId]);
  if (!node) {
    return html`
      <div class="callout warn">Схема для настройки канала недоступна.</div>
    `;
  }
    ...
    <div style="margin-top: 16px;">
      ${
        props.configSchemaLoading
          ? html`
              <div class="muted">Загружаем схему конфига…</div>
            `
      ...
      <div class="row" style="margin-top: 12px;">
        ...
        >
          ${props.configSaving ? "Сохраняем…" : "Сохранить"}
        </button>
        <button
          class="btn"
          ?disabled=${disabled}
