...
const telegramAccountSnapshotOnlyPlugin: ChannelPlugin = {
  id: "telegram",
  meta: {
    id: "telegram",
    label: "Telegram",
...
describe("gateway server channels", () => {
  ...
  test("channels.status mirrors default account runtime fields when summary is missing", async () => {
    setRegistry(
      createRegistry([
        {
          pluginId: "telegram",
          source: "test",
          plugin: telegramAccountSnapshotOnlyPlugin,
        },
      ]),
    );
    const res = await rpcReq<{
    ...
    expect(telegram?.lastProbeAt).toBe(123);
    expect(telegram?.lastStartAt).toBe(456);
    expect(telegram?.lastError).toBeNull();
  });
});
