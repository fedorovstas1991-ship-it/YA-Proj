...
import { hasPlaintextConfigSecrets } from "../config/secrets.js";
...
export async function startGatewayServer(
  ...
  if (hasPlaintextConfigSecrets(configSnapshot.config)) {
    await writeConfigFile(configSnapshot.config);
    log.info("gateway: migrated plaintext config secrets into keychain-backed secret refs");
    configSnapshot = await readConfigFileSnapshot();
  }

  // Proactively triggers auth-profiles plaintext -> secretRef migration at startup.
