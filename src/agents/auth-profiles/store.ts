...
import { externalizeAuthProfileStoreSecrets, hydrateAuthProfileStoreSecrets } from "./secrets.js";
...
export function loadAuthProfileStore(): AuthProfileStore {
  ...
  if (asStore) {
    ...
    const externalized = externalizeAuthProfileStoreSecrets(asStore);
    const finalStore = externalized.store;
    if (synced || externalized.mutated) {
      saveJsonFile(authPath, finalStore);
    }
    return hydrateAuthProfileStoreSecrets(finalStore);
  }
  ...
  syncExternalCliCredentials(store);
  const externalized = externalizeAuthProfileStoreSecrets(store);
  if (externalized.mutated) {
    saveJsonFile(authPath, externalized.store);
    return hydrateAuthProfileStoreSecrets(externalized.store);
  }
  return hydrateAuthProfileStoreSecrets(store);
}
...
function loadAuthProfileStoreForAgent(
  ...
  const externalized = externalizeAuthProfileStoreSecrets(store);
  const finalStore = externalized.store;
  const shouldWrite = legacy !== null || mergedOAuth || syncedCli || externalized.mutated;
  if (shouldWrite) {
    saveJsonFile(authPath, finalStore);
  }
...
export function saveAuthProfileStore(store: AuthProfileStore, agentDir?: string): void {
  const authPath = resolveAuthStorePath(agentDir);
  const externalized = externalizeAuthProfileStoreSecrets(store);
  const storeToSave = externalized.store;
  const payload = {
    version: AUTH_STORE_VERSION,
    profiles: storeToSave.profiles,
    order: storeToSave.order ?? undefined,
    lastGood: storeToSave.lastGood ?? undefined,
    usageStats: storeToSave.usageStats ?? undefined,
