    ...
    try {
      ...
      try {
        ...
        try {
          ...
          log.error(`[DEBUG attempt] About to call activeSession.prompt for runId=${params.runId}`);
          if (imageResult.images.length > 0) {
            await abortable(activeSession.prompt(effectivePrompt, { images: imageResult.images }));
          } else {
            await abortable(activeSession.prompt(effectivePrompt));
          }
          log.error(`[DEBUG attempt] activeSession.prompt returned for runId=${params.runId}`);
        } catch (err) {
          promptError = err;
        } finally {
          log.debug(
