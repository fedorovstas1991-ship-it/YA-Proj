...
describe("subscribeEmbeddedPiSession", () => {
  ...
  it("emits lifecycle error when final assistant stopReason is error", () => {
    let handler: ((evt: unknown) => void) | undefined;
    const session: StubSession = {
      subscribe: (fn) => {
        handler = fn;
    ...
    const last = lifecycleEvents.at(-1);

    expect(last?.data?.phase).toBe("error");
    expect(last?.data?.error).toBe("Connection error.");
  });
