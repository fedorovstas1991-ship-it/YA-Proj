...
import { setOpenrouterApiKey } from "../commands/onboard-auth.credentials.js";
...
} from "../onboarding/kimi-preset.js";
...
export function applyElizaOpenrouterKimiProviderConfig(cfg: OpenClawConfig): OpenClawConfig {
...
async function runElizaOnboardingWizard(params: {
  ...
  const apiKey = await params.prompter.text({
    message: `Откройте ${QUOTA_URL}, нажмите "Получить токен" и вставьте его`,
    placeholder: "y1__xDov6eRpdT...",
    sensitive: true,
    validate: (value) =>
      isLikelyYandexQuotaToken(String(value ?? "").trim())
        ? undefined
        : "Нужен токен из quota (формат y1_...)",
  });

  const agentDir = resolveOpenClawAgentDir();
  await setOpenrouterApiKey(String(apiKey).trim(), agentDir);

  // Patch config to use OpenRouter provider routed via Eliza endpoint + default Kimi 2.5.
  const snapshot = await readConfigFileSnapshot();
  const baseConfig: OpenClawConfig = snapshot.valid ? snapshot.config : {};
  let nextConfig = applyElizaOpenrouterKimiProviderConfig(baseConfig);
