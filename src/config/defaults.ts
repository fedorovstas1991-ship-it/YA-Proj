...
export function applyCompactionDefaults(cfg: OpenClawConfig): OpenClawConfig {
  ...
  // Проверяем какие настройки нужно применить (merge-логика)
  const shouldApplyMode = existingCompaction.mode === undefined;
  ...
  // Если все настройки уже есть - ничего не меняем
  if (!shouldApplyMode && !shouldApplyReserve && !shouldApplyKeep && !shouldApplyFloor && !shouldApplyBootstrap) {
    return cfg;
  }
  ...
  return {
    ...
    agents: {
      ...
      defaults: {
        ...
        compaction: {
          ...existingCompaction,
          ...(shouldApplyMode && { mode: "safeguard" }),
