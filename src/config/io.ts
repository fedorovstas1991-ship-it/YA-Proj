...
import { getSecretStore, type SecretStore } from "../infra/secrets/store.js";
...
import { externalizeConfigSecrets, hydrateConfigSecretRefs } from "./secrets.js";
...
function normalizeDeps(overrides: ConfigIoDeps = {}): Required<ConfigIoDeps> {
  ...
  return {
    ...
    env,
    homedir:
      overrides.homedir ?? (() => resolveRequiredHomeDir(env, os.homedir)),
    configPath: overrides.configPath ?? "",
    secretStore: overrides.secretStore ?? getSecretStore({ env }),
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OpenClawConfig {
    try {
      ...
      const runtimeConfig = applyConfigOverrides(cfg);
      return hydrateConfigSecretRefs(runtimeConfig, deps.secretStore);
    } catch (err) {
      ...
      if (
        code === "OPENCLAW_SECRET_MISSING" ||
        code === "OPENCLAW_SECRET_STORE_UNAVAILABLE" ||
        (err instanceof Error && err.message.includes("Secret storage backend disabled"))
      ) {
        throw err;
      }
  ...
  async function readConfigFileSnapshot(): Promise<ConfigFileSnapshot> {
    ...
    if (!exists) {
      ...
      const config = applyDefaultMcpServers(
        applyTalkApiKey(
          applyModelDefaults(
            applyCompactionDefaults(
              applyContextPruningDefaults(
  ...
  async function writeConfigFile(cfg: OpenClawConfig) {
    ...
    const serializedConfig = externalizeConfigSecrets(
      applyModelDefaults(stampConfigVersion(cfg)),
      deps.secretStore,
    );
    const json = JSON.stringify(serializedConfig, null, 2)
      .trimEnd()
      .concat("\n");
