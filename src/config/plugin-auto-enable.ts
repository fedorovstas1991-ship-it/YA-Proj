...
function isBundledChatChannel(pluginId: string): boolean {
  return Boolean(normalizeChatChannelId(pluginId));
}

function registerPluginEntry(cfg: OpenClawConfig, pluginId: string): OpenClawConfig {
  // Skip creating plugin entries for bundled chat channels (telegram, discord, etc.)
  // These are not plugins and don't need entries in plugins.entries
  if (isBundledChatChannel(pluginId)) {
    return cfg;
  }
...
export function applyPluginAutoEnable(params: {
  ...
  for (const entry of configured) {
    ...
    // Skip bundled chat channels - they don't need plugin entries
    if (isBundledChatChannel(entry.pluginId)) {
      continue;
    }
