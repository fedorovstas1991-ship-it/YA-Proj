...
### memory-core daily logs, promotion, and recent index

When the `memory-core` plugin is active, it can capture day-to-day notes into a
separate daily log, then periodically promote durable items into `MEMORY.md`.
Configure it under `plugins.entries.memory-core.config`.

Key behaviors:

- **Daily log location**: `~/.openclaw/agents/<agentId>/memory/YYYY-MM-DD.md`.
  The plugin appends entries to this file based on the configured capture rules.
- **Promotion cadence**: `promote.intervalHours` controls how often the plugin
  reviews the daily log and moves durable items into `MEMORY.md`.
- **Clarification gate**: `clarify.minMessages` sets the minimum new messages
  before the classifier asks for clarification or promotes notes.
- **Recent index**: the plugin maintains `memory/recent/` in the workspace as a
  rolling window of hardlinks to recent daily log files. This gives QMD a stable
  path for “recent memory” without indexing every historical log.

Example configuration (memory-core + QMD recent index):
...
plugins: {
  ...
  entries: {
    "memory-core": {
      ...
      config: {
        timezone: "UTC",
        promote: { intervalHours: 6, aggressiveness: "broad" },
...
memory: {
  ...
  qmd: {
    ...
    paths: [
      { name: "memory-recent", path: "./memory/recent", pattern: "**/*.md" },
