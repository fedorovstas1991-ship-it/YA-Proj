...
- [ ] **Проблема:** Скрипт удаляет только директорию состояния gateway. Он не отправляет команды через API для очистки сессий и агентов, которые могут быть сохранены где-то еще, если архитектура это позволяет (например, в базе данных). Однако, удаление `$STATE_DIR` должно по идее быть полным сбросом для локального gateway. Если предполагается более глубокий сброс (например, для удаленного gateway), то потребуется API вызов.
- [ ] **TODO:** Дополнительно к `rm -rf "$STATE_DIR"` добавить вызов API, если это применимо к целевой архитектуре (например, для удаленного OpenClaw gateway). Если `rm -rf` уже достаточно для _локального_ gateway, то этот пункт может быть пропущен.
...
- [ ] **Проверить URL параметры:**
  - **Текущая строка:** `URL_UI="http://127.0.0.1:${PORT}/?simple=1&onboarding=1#token=${TOKEN}&gatewayUrl=ws://127.0.0.1:${PORT}"`
  - **TODO:** Параметр `onboarding=1` в URL уже должен приводить к показу онбординга. В сочетании с очисткой `localStorage` это должно гарантировать, что онбординг запускается с первого шага.
- [ ] **Сценарий тестирования:**
  - [ ] Запустить UI.
  - [ ] Пройти часть онбординга.
  - [ ] Закрыть UI.
  - [ ] Запустить скрипт `yagent-onboard-ui.command`.
  - [ ] Проверить, что UI открывается и онбординг начинается с первого шага.
...
- [ ] Персоны агентов: темперамент, стиль общения
- [ ] Базы знаний: подключить документы/Wiki к агенту
- [ ] Версионирование агентов: история промптов, rollback
- [ ] Расписание задач: cron-задачи для агентов с мониторингом
- [ ] Маркетплейс агентов и скиллов (шаринг конфигов)
- [ ] Мониторинг затрат: расходы по агентам/проектам, бюджет лимиты
- [ ] AI-помощник для написания промптов (автодополнение, шаблоны, оценка)
- [ ] Командные рабочие пространства: роли, совместный доступ
- [ ] Интегрированная code sandbox для выполнения кода агентом
...
- [ ] Магический Бриф: из одной фразы → задача + план + чек-лист + ожидаемые результаты
- [ ] Автораспределение задач: AI распределяет работу по людям с учётом навыков и загрузки
- [ ] Единое окно статусов: сводка всех проектов — прогресс, риски, блокеры
- [ ] Шаблоны "Сделай как в прошлый раз": копирует структуру из похожего проекта
- [ ] Командный радар здоровья: оценка загрузки, фокуса и рисков выгорания
